...
orgs-own [
  ...
  weatherImpactExp
  impactThreshold  ; above which impacts will be constitute punctuation to trigger revisiting risk perception
  otherInfExp
...
to setup-regionalRiskThreshold
  (foreach ["northeast" "midwest" "south" "west"][0.40 0.45 0.30 0.50]
    [
      [x meanThreshold] ->
      ask turtles with [region = x] [set riskPerceptionThreshold random-normal meanThreshold 0.05]
...
to setup-solutions ; every turtle begins with a solution
  ask orgs [
    ...
    ; after determine solution, generate the impact norm
;    set impactNorm record-impact-exp 1 extremeweathe
  ]
...
to record-weather-norm
  ask orgs [
      ...
      set weatherSeverityExp item 0  weatherSeverityImpactExp
      set weatherImpactExp item 1 weatherSeverityImpactExp
      set extremeWeatherThreshold  item (ceiling simulateYears * extremeWeatherProb) weatherSeverityExp
;      print weatherSeverityExp
;      set severityMax max weatherSeverityExp
      set disasterThreshold  item (ceiling simulateYears * disasterProb) weatherSeverityExp
      set impactThreshold item (ceiling simulateYears * badImpact) weatherImpactExp
      if impactThreshold = 0 [set impactThreshold 0.1]
  ]
end
...
to take-impact
  ...
  ifelse perceived-control?
  [set buffer sol-efficacy * resilience ]
  [set buffer sol-efficacy * resilience]


  ifelse buffer  >= weatherSeverity ; impact is jointly decided by efficacy of org solution and weather severity
  [set impactPerTick 0]
  [set impactPerTick weatherSeverity - buffer]
...
to total-riskPerception
  ask orgs [
    if impactPerTick > impactThreshold [  ; risk perception is updated on a punctuated manner after experiencing big impacts from extreme weather
...
to-report record-severity-experience[ewProb org-sol-efficacy org-resilience]
  ...
  while [time < simulateYears][  ; base weather pattern on 200 year experience
    let severityPerTick log-normal 1 ewProb
...
"" ""
PENS
"riskPer" 1.0 0 -15040220 true "" "plot [riskPerceptionSum] of one-of orgs with-max [riskPerceptionThreshold]"
...
"" ""
PENS
"RiskPer" 1.0 0 -14439633 true "" "plot sum [riskPerceptionSum] of  orgs with-max [extremeWeatherProb]"
...
"" ""
PENS
"NE" 1.0 0 -15040220 true "" "plot sum [riskPerceptionSum] of orgs with-max [initialResilience]"
...
"" ""
PENS
"riskPer" 1.0 0 -15040220 true "" "plot [riskPerceptionSum] of one-of orgs with-min [riskPerceptionThreshold]"
...
"" ""
PENS
"default" 1.0 0 -14439633 true "" "plot sum [riskPerceptionSum] of orgs with-min [extremeweatherprob]"
...
"" ""
PENS
"default" 1.0 0 -14439633 true "" "plot sum [riskperceptionsum] of orgs with-min [initialresilience]"
