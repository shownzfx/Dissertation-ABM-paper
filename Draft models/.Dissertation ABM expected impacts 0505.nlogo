...
to setup-regionalRiskThreshold
  ...
  (foreach ["northeast" "midwest" "south" "west"][0.35 0.40 0.25  0.45]
    [
      [x meanThreshold] ->
      ask turtles with [region = x] [set riskPerceptionThreshold random-normal meanThreshold 1]
...
to setup-orgs
  ...
  ask orgs [
    ...
    set shape "circle"
    set size 0.6
    set initialResilience 0
    set riskPerception 0 ; risk perception for each tick
    set extremeWeatherFreq 0
    set otherInfExp []
    set expectedExtremeWeatherProb 0
    set expectedImpact 0
...
;    set resilience random-normal 4 1
;    if resilience < 0 [set resilience 0.5]
    set initialResilience resilience
    set riskPerceptionThreshold random-normal meanRiskThreshold sdRiskThreshold
    if riskPerceptionThreshold < 0.2 [set riskPerceptionThreshold  random-float 0.5 ]
...
to set-efficacy-cost
  set efficacy random-float 0.05 + random-float (1 - 0.05); for incremental solutions
...
to record-weather-norm
  ask orgs [
      ...
      set weatherSeverityImpactExp record-severity-experience extremeWeatherProb resilience sol-efficacy; determine weather severity norm based on 200 years timeline, functions are put at the end
...
;      set severityMax max weatherSeverityExp
      ...
      set expectedImpact (expectedBadWeatherSeverity - (resilience * sol-efficacy)) * extremeWeatherProb
      if expectedImpact < 0 [set expectedImpact 0.1]
;      if expectedBadWeatherSeverity = 0 [set expectedBadWeatherSeverity 0.01]
  ]
end

to-report record-severity-experience [ewProb org-sol-efficacy org-resilience]
  let time 0
  let severity []
  let impactRecord[]
  ...
  while [time < simulateMonths][  ; base weather pattern on 200 year experience
    ...
    let myImpact SeverityPerTick - org-resilience * org-sol-efficacy
...
to check-weather
  ask orgs [
    ...
    if weatherSeverity > expectedBadWeatherSeverity [set expectedBadWeatherSeverity weatherSeverity]
...
;    set weatherSeverity tempSeverity / severityMax ; scaling severity within 0 and 1
    ifelse weatherSeverity > extremeWeatherThreshold [
      ...
      set expectedExtremeWeatherProb extremeWeatherProb * (1 + random-float 0.05) ; if extremeweather event happened, then expectedProb increasese
      take-impact
    ...
    [set expectedExtremeWeatherProb extremeWeatherProb * (1 - random-float 0.05)] ; extremeweatherevent did not happen, then expectedprob decreases
  ]
...
;  let effectiveIncrementalChange filter [x -> ticks - x < 60] incrementalChangeTicks
  ...
  set buffer sol-efficacy * resilience
  set impactPerTick ifelse-value (weatherSeverity - buffer < 0 ) [0] [weatherSeverity - buffer]


  set expectedImpact (expectedBadWeatherSeverity - (sol-efficacy * resilience)) * expectedExtremeWeatherProb
...
to update-weatherExp
  ask orgs [
    ...
    if (not empty? impactExp) and (ticks > orgMemory) [  ;can only remember risk within the institutional memory
      set impactExp filter [x -> ticks - item 2 x < orgMemory and item 1 x > 0] impactExp  ; filter out events orgs beyond the org memory and events that cause zero impact
    ]
  ]
end
...
to update-riskExp-from-others
  ask orgs [
    set neighborsWithDisaster org-sameReg-link-neighbors with [disaster? and impactPerTick > 0]
...
to total-riskPerception
  ask orgs [
    if impactPerTick > 0 [  ; risk perception is updated on a punctuated manner after experiencing big impacts from extreme weather
...
to search-solution
  ask orgs [
    if expectedImpact > riskPerceptionThreshold [
      let sol-efficacy [efficacy] of current-solution
      let currentExpectedImpact expectedBadWeatherSeverity - (resilience  * sol-efficacy)
      if currentExpectedImpact < 0 [set currentExpectedImpact 0]

      if currentExpectedImpact > 0[
         let targetSolEfficacy calculate-target-efficacy sol-efficacy resilience currentExpectedImpact expectedBadWeatherSeverity extremeWeatherProb impactReductionRate
...
to-report  calculate-target-efficacy [org-sol-efficacy org-resilience org-impact org-weatherSeverity org-extremeWeatherProb org-reductionRate]
 let orgExpectedImpact org-impact * (1 - org-reductionRate)
...
;  [orgs with [not innovation-ready? and length incrementalChangeTicks = 0 ]]
;
;  ask targetOrgs [
;    if expectedImpact > riskPerceptionThreshold [
;      set satisfied? false
...
to riskPerception-decay
  ask orgs [
    if not extremeWeather?[
      set riskPerceptionSum (riskPerceptionSum * perceptionDecay)
    ]
  ]
end
...
"pen-2" 1.0 0 -16777216 true "" "plot count orgs with [innovation-ready?]"
...
485
#inno
count orgs with [innovation-ready?]
0
1
11
...
"" ""
PENS
"NE" 1.0 0 -15040220 true "" "plot sum [expectedImpact] of orgs with-max [initialResilience]"
"threshold" 1.0 0 -8053223 true "" "plot sum [riskPerceptionThreshold] of orgs with-max [initialResilience]"
...
"" ""
PENS
"default" 1.0 0 -14439633 true "" "plot sum [expectedImpact] of orgs with-min [initialresilience]"
"pen-1" 1.0 0 -8053223 true "" "plot sum [riskperceptionthreshold] of orgs with-min [initialresilience]"
...
"" ""
PENS
"ExpImp" 1.0 0 -14439633 true "" "plot sum [expectedImpact] of orgs with-min [resilience]"
"pen-1" 1.0 0 -10873583 true "" "plot sum [riskPerceptionThreshold] of orgs with-min [resilience]"
...
380
EEWProb
sum [expectedExtremeWeatherProb] of orgs with-min [resilience]
5
1
11
...
435
weatherSev
sum [weatherSeverity] of orgs with-min [resilience]
5
1
11
...
480
expectedSev
sum [expectedBadWeatherSeverity] of orgs with-min [resilience]
5
1
11
...
475
orgSolEff
sum [[efficacy] of current-solution] of orgs with-min [resilience]
5
1
11
