...
to setup-orgs
  ...
  ask orgs [
    ...
    set resilience random-float maxResilience
    set satisfied? true
...
to go
  ...
  update-weatherExp
  perceive-risk
  ; perceive-risk-from-others ; not sure if i want to incorporate risk perception arising from observing others' going through extreme weather
  search-solution

  ask orgs [set extremeWeather? false]
...
to update-riskPerception
   let riskperceptionSum1 0
   let riskPerceptionUpdate (item 0 freqImpact) * 0.2 + (item 2 freqImpact) * 0.26 + random-normal 0 0.01  ; add some random errors
   set riskperceptionSum1 riskperceptionSum1 + riskPerceptionUpdate
   set riskPerceptionSum riskperceptionSum1
...
to search-solution
  ask orgs with [satisfied?] [
    if riskPerceptionSum > riskPerceptionThreshold [
      ...
      let known-solutions (turtle-set [org-sol-link-neighbors with [not adaptation? ] ] of myneighbors)
      ...
      ifelse any? better-solutions
       [
         let target-solution one-of better-solutions
  ...
  if random-float 1 < innoRate [
   ask patch-here [
      ...
      [
        ...
        set efficacy 0.5 + random-float (1.5 - 0.5)
      ]
    ]

    let target-solution one-of solutions-here with [adaptation?]
