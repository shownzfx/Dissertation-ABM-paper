...
to import-orgs
  ...
  [
    ...
    create-orgs  1 [
      ...
      set maxCopingEfficacy  maxCopingReduction * expectedBadWeatherSeverity + solEfficacy ; the maximum risk reduction coping measures can acheive,do not multiple ewprob
...
to setup-windows
  ask orgs [
    repeat random numWindows [
      let changeEvent random 1000 ; 1000 ticks
...
to go
  ...
  if ticks mod 600 = 0 [
  ask orgs [
    set changeEvents update-windows
  ]
 ]
...
to-report update-windows
  let updatedEvents []
  repeat random numWindows [
    let updatedEvent random 600 + ticks
    set updatedEvents fput updatedEvent changeEvents
  ]
...
;  if org-target-efficacy < org-solEfficacy [print "warning: org target efficacy is smaller than current efficacy"]
...
;  print is-turtle-set? [projectInventory] of FTAoffice-org-link-neighbors
;  let knownSolutions4 [projectInventory] of FTAoffice-org-link-neighbors


;  set knownSolutions (turtle-set knownSolutions1 knownSolutions2 knownSolutions3 knownSolutions4)
;
;
...
to check-adaptation
  ...
  ask FTAoffices[
    set projectInventory ([current-solution] of FTAoffice-org-link-neighbors)
    set projectInventory filter [ x -> [adaptation?] of x] projectInventory
  ]
end
