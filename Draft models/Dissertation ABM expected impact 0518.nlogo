...
to import-orgs
  ...
  [
    ...
    create-orgs  1 [
      ...
      set disasterProb disasterProb + random-float 0.025
  ...
  (foreach ["northeast" "midwest" "south" "west"] regionalMean
    [
      ...
      ask turtles with [region = x] [
        ...
        if riskPerceptionThreshold < 0 [set riskPerceptionThreshold 0.0]
      ]
    ])
end
...
to go
  ...
  search-solution
  check-window
  check-adaptation ; this is the FTAoffice procedure
...
to assess-thruNetwork
  ...
  let knownSolutions (turtle-set knownSolutions1 knownSolutions2 knownSolutions3)
...
;    print [adaptation?] of targetSolution
...
to check-adaptation
  ...
  ask FTAoffices[
    let projects  (turtle-set [current-solution] of FTAoffice-org-link-neighbors) with [adaptation?]
    if any? projects [
      set projectInventory sentence projects projectInventory
    ]
end
