...
to import-orgs
  ...
  [
    ...
    create-orgs  1 [
      ...
      set passRate (item 5 row + random-float 0.02 ) * (1 / 24)
...
to setup-orgs
  ask orgs [
    ...
    set maxCopingEfficacy  maxCopingReduction * worstWeatherIntensity  ; the maximum risk reduction coping measures can acheive
  ]
...
to setup-solutions ; every turtle begins with a solution
  ...
  ask n-of 30 patches with [not any? turtles-here and pcolor != grey][
    sprout-solutions 1 [
      set color yellow ; adaptation are set blue
      set efficacy 2 + random-float 2 ;weatherIntensity ranges from approximately  2 to 4
...
to record-weather-norm ; only activated when not using the hard coded weather parameters on the following attributes
  ask orgs [
      ...
      if extremeWeatherProb < disasterProb [
         print "extremeWeatherProb is smaller than disasterProb"
         set extremeWeatherProb disasterProb + 0.03
    ]
...
to-report record-severity-experience
  ...
  while [time < simulateMonths][  ; base weather pattern on 200 year experience
    let severityPerTick log-normal regionIntensityMean  0.1;
...
;  profile:start
  ...
  if not randomOrgOpt? [generate-orgOpportunities]
...
to search-solution
 ...
 ;     let targetSolEfficacy calculate-target-efficacy solEfficacy impactPerTick worstWeatherIntensity  (random-float impactReductionRate + 0.1)
...
to-report  calculate-target-efficacy [org-solEfficacy org-weatherIntensity  org-impact  org-reductionRate]
...
"threshold" 1.0 0 -8053223 true "" "plot  [riskTolerance] of one-of orgs with-max [originalEfficacy]"
...
"Thresh" 1.0 0 -8053223 true "" "plot sum [riskTolerance] of orgs with-min [originalEfficacy]"
