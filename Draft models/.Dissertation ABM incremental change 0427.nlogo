...
to setup-orgs
  ...
  ask orgs [
    ...
    set riskPerceptionThreshold random-normal 1.5 1
...
to filter-othersInf
  ask orgs [
    if not empty? otherInfExp [
      if ticks > orgMemory [
        set otherInfExp filter [x -> ticks - item 1 x < orgMemory  ] otherInfExp
...
to perceive-risk-from-others
  ask orgs [
    if not empty? otherInfExp [
    set riskPerceptionFromOthers 0
    let riskPerceptionFromOthersUpdate sum ( map [x -> 1 * 0.1 + item 1 x * 0.15 + random-normal 0 0.01] otherInfExp)
...
to search-solution
  ask orgs with [riskPerceptionSum > riskPerceptionThreshold ][
    set satisfied? false
    ...
    [
        ...
        set color blue
        set incrementalChangeTime fput ticks incrementalChangeTime
    ]
    [
      set innovation? true
    ]
