...
solutions-own [efficacy originalEfficacy cost to-opportunity adaptation?]; solutions include both non-adaptation and adaptation measures
FTAoffices-own []
...
to setup-regionalRiskThreshold
  let regionalMean1 sentence (meanRiskThreshold + 0.10) (meanRiskThreshold + 0.15)  ; threshold for northeast and midwest
  let regionalMean2 sentence (meanRiskThreshold + 0) (meanRiskThreshold + 0.2) ; threshold for south and west
...
to set-efficacy-cost
  set efficacy 0.05 + random-float (1 - 0.05); for incremental solutions
  set originalEfficacy efficacy
...
;    if weatherSeverity > expectedBadWeatherSeverity [set expectedBadWeatherSeverity weatherSeverity]
    ...
    ]  ; disaster more strongly enhance expected EW prob
     if expectedEWProb >= 0.25 [set expectedEWProb  0.25]
...
to expect-impact
  ask orgs [
    ...
    set expectedImpact (expectedBadWeatherSeverity - (sol-efficacy * resilience)) * expectedEWProb
...
to search-solution
  ask orgs with [expectedImpact > riskPerceptionThreshold] [
    if searchCost <= searchCostThreshold [
      set searchCost searchCost + 1
      let original-sol-efficacy [originalEfficacy] of current-solution
      let sol-efficacy [efficacy] of current-solution
      let currentImpact expectedBadWeatherSeverity - (resilience  * sol-efficacy)
      if currentImpact < 0 [set currentImpact 0]

      if currentImpact > 0[
         let targetSolEfficacy calculate-target-efficacy sol-efficacy resilience currentImpact expectedBadWeatherSeverity  (random-float impactReductionRate)
         if targetSolEfficacy / sol-efficacy < 1.5 [
...
to-report  calculate-target-efficacy [org-sol-efficacy org-resilience org-impact org-weatherSeverity  org-reductionRate]
...
"RealImp" 1.0 0 -10873583 true "" "plot (sum [impactPerTick] of orgs with-min [resilience]) * 0.1"
...
380
EWProb
sum [extremeWeatherProb] of orgs with-max [extremeweatherProb]
5
1
11
...
435
weatherSev
sum [weatherSeverity] of orgs with-max [extremeweatherProb]
5
1
11
...
480
expectedProb
sum [expectedEWProb] of orgs  with-max [extremeweatherProb]
5
1
11
...
475
orgSolEff
sum [[efficacy] of current-solution] of orgs with-max [extremeweatherProb]
5
1
11
