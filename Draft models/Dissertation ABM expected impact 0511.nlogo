...
to setup-solutions ; every turtle begins with a solution
  ask orgs [
    ask patch-here [
      ...
      [
         ...
         set cost sum [capacity] of my-org - 0.01
  ...
  ask n-of 30 patches with [not any? turtles-here and pcolor != grey][
    sprout-solutions 1 [
      set color blue
      set cost random-float adaptionCost + 0.1
      set efficacy 2 + random-float 2 ; ranges from 2 to 4
...
to search-solution
  ask orgs with [not solution-ready?][
...
to search-adaptation
    let adaptationPool solutions with [adaptation?]
    set targetSolution one-of adaptationPool with-max [efficacy]
    if  targetSolution != nobody [set solution-ready? true]
...
to check-window
  if open-windows? [
    ask orgs with [postponed? and expectedImpact > riskPerceptionThreshold][
      if member? ticks changeEvents [
...
"" ""
PENS
"toAdapt" 1.0 0 -5298144 true "" "plot count orgs with [search-adaptation?]"
...
"toAdapt" 1.0 0 -10873583 true "" "plot count orgs with [search-adaptation?]"
...
380
solEff
sum [solEfficacy] of orgs with-max [extremeweatherProb]
5
1
11
