...
;  (foreach FTARegions [
;    x ->
;    let ftaAgencies orgs with [ftaRegion = x]
;    ask ftaAgencies [
;      set tempXcorList sentence tempXcorList ([pxcor] of patch-here)
;      set tempYcorList sentence tempYcorList ([pxcor] of patch-here)
;    ]
;    set tempXcor mean tempXcorList
;    set tempYcor mean tempYcorList
;
;    ask patches with [pxcor = tempXcor and pycor = tempYcor] [
;      sprout-FTAoffices 1 [
;        set shape "house"
;        set size 2
;      ]
...
;    ])
  ...
  (foreach FTARegions [
    x ->
    ask [patch-here] of  one-of orgs with [ftaRegion = x][
      sprout-FTAoffices 1 [
        set shape "house"
        set size 2
        set color yellow
...
to avoid-edges
    while [[pcolor] of patch-here = grey or any? orgs-here][fd 0.4]
...
;    set resilience random-float maxResilience  ;resilience is the ax magnitude of disturbances that can be tolerated before incurring impacts
...
to check-weather ; param for frequency of extreme weather comes from the survey
  ask orgs [
    ...
    if extremeWeatherProb  < disasterProb [ ; 5% chance for weather disaster
      set disaster? true
    ]
  ]
...
to take-impact
  ...
  set extremeWeatherSeverity random-float 2
  let sol-efficacy sum [efficacy] of org-sol-link-neighbors
  ifelse sol-efficacy >= extremeWeatherSeverity ; impact is jointly decided by efficacy of org solution and weather severity
  [set impactPerTick 0]
