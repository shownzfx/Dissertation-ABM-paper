...
to check-weather ; param for frequency of extreme weather comes from the survey
  ask orgs [
    ...
    while [extremeWeatherProb >= disasterProb] [ ; make sure extreme weather prob is bigger than disasterProb
      set extremeWeatherProb random-float 1
    ]

    if random-float 1 < disasterProb [
      set disaster? true
    ]
  ]

  ask orgs with [region = "northeast" or region = "midwest"] [
    ifelse extremeWeatherProb  >= 0.75 ; 25% of the time , the agency has extreme weather events
