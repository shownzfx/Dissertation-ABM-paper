...
to setup-orgs
  ...
  ask orgs [
    ...
    set size 0.6
    set shape "circle"
    set capacity random-float 0.05 0.95;
...
to setup-orgNetwork ; orgs in the four regions generate networks with orgs in the same and in the different regions
  ...
  (foreach ["Northeast" "Midwest" "South" "West"]
    [
      ...
      create-links-with n-of random 12 other turtles with [region = x] ; designed so there are more neighbors within than outside the regions
...
to set-cost-and-color  ;solutions differ in costs (adaptation are more expensive)
    ifelse random-float 1 <= 0.2
      [
        ...
        set efficiency random-float 0.05 0.95
        set color green
...
;***************modules to add***************
ask turtles [
  let myNeighbor one-of other turtles
  compare-with myNeighbor
  ask myNeighbor [compare-with myself]
]

to compared-with [other-turtle]
  let other-utility [utility] of other-turtle
  let u-diff [utility - other-utility] / (abs utility + abs other-utility)
  if (u-diff < 0 and random-float 1.0 < abs u-diff) [
    switch-strategy
  ]
end

to switch-strategy
  set s-index [s-index + 1] mod 2
  set color item s-index strat-colors
  set effort item s-index efforts
  ; set efforts (list (effort-opt/ num-agents) (efforts-opt/num-agents*mu))
  ; s-index takes value 0 and 1
end

to set-leader
  let average-links 0
  set average-links links-to-be-leader
  ask turtles with [count link-neighbors] [round average-links]
  [
    set shape "circle"
    set breed leader
