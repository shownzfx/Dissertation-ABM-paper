...
to record-weather-norm
  ask orgs [
      let sol-efficacy [efficacy] of org-sol-link-neighbors
      set weatherSeverityExp record-severity-experience extremeWeatherProb resilience sol-efficacy; determine weather severity norm based on 200 years timeline, functions are put at the end
...
to-report record-severity-experience[ewProb org-sol-efficacy org-resilience]
  ...
  while [time < 2400][  ; base weather pattern on 200 year experience
    ...
    set severity fput severity severityPerTick
    let severityImpact SeverityPerTick - org-resilience * org-sol-efficacy
    set impactRecord fput severityImpact impactRecord
    set time time + 1
  ]
