...
to setup-orgs
  ...
  ask orgs [
    ...
    set riskPerceptionThreshold random-normal 0.2 0.05
...
;    set weatherSeverity tempSeverity / severityMax ; scaling severity within 0 and 1
    if tempSeverity > extremeWeatherThreshold [
...
to perceive-risk-from-others
  ask orgs [
    if not empty? otherInfExp [
    set riskPerceptionFromOthers 0
    let riskPerceptionFromOthersUpdate sum ( map [x -> 1 * 0.03 + item 1 x * 0.065 + random-normal 0 0.01] otherInfExp)
...
;        set target-solution ifelse-value (random-float 1 <= 0.5) [one-of better-solutions][one-of better-solutions with-max [efficacy]]
        ...
        set perceivedControl perceivedControl + random-float 0.2
        set color blue
