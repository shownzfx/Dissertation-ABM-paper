  ...
  (foreach FTARegions [
    ...
    ask [patch-here] of one-of FTAorgs [
      sprout-FTAoffices 1 [
        ...
        set color yellow
        avoid-edges
        create-FTAoffice-org-links-with FTAorgs
      ]
    ]
  ])
...
to setup-orgs
  ...
  ask orgs [
    ...
    set otherInfExp []
    set resilience random-normal 0 2 ;resilience is the ax magnitude of disturbances that can be tolerated before incurring impacts, can be negative
    set riskPerceptionThreshold random-float perceptionThreshold
    set satisfied? true
...
to check-weather ; param for frequency of extreme weather comes from the survey
  ...
  ask orgs with [region = "northeast" or region = "midwest"] [
    ifelse extremeWeatherProb <= 0.25; 25% of the time , the agency has extreme weather events
    [take-impact]  ; the frequency 0.75 is drawn from the survey data
    [set impactPerTick 0]

  ]
  ask orgs with [region = "south"][
     ifelse extremeWeatherProb >= 0.2
     [take-impact]
     [set impactPerTick 0]
  ]
  ask orgs with [region = "west"] [
    ifelse extremeWeatherProb  <= 0.15
    [take-impact]
    [set impactPerTick 0]
  ]
...
to take-impact
  ...
  ifelse sol-efficacy * resilience >= extremeWeatherSeverity ; impact is jointly decided by efficacy of org solution and weather severity
  [set impactPerTick 0]
  [set impactPerTick extremeWeatherSeverity - sol-efficacy * resilience]
...
to perceive-risk-from-exp
  ask orgs [
    ...
    let riskPerceptionUpdate sum (map [x -> item 0 x * 0.2 + item 1 x * 0.27 + random-normal 0 0.01] impactExp)
...
to update-riskExp-from-others
  ask orgs [
    let neighborWithDisaster org-sameReg-link-neighbors with [disaster?]
    if any? neighborWithDisaster[
      set neighborWithDisaster (list neighborWithDisaster)
      let disasterInfluence (map[x -> list x ticks] neighborWithDisaster)
...
to filter-othersInf
  ask orgs [
    if not empty? otherInfExp [
      if ticks > memoryAbtOthers [
        set otherInfExp filter [x -> ticks - item 1 x < memoryAbtOthers] otherInfExp
        let temp (map [x -> item 1 x] otherInfExp)
      ]
    ]
  ]
...
to perceive-risk-from-others
  ask orgs [
    set riskPerceptionFromOthers 0
    let #affectedNeighbors length otherInfExp
    set riskPerceptionFromOthers #affectedNeighbors * 0.1
    ; 0.02 (inf on risk perception from EW frequency of others) is set to be much lower than (impact from EW orgs directly experienced)
  ]
...
to search-solution
  ask orgs with [satisfied? and color != blue] [
    if riskPerceptionSum > riskPerceptionThreshold [
      ...
      ifelse any? better-solutions
       [
         ...
         create-org-sol-link-with target-solution [hide-link]
...
to setup-innovation
  create-solutions 20 [
    ...
    set efficacy 0.3 + random-float (1 - 0.3) ;the routine solution efficacy is set at random-float 0.5; adaptatione efficacy is higher
...
"pen-1" 1.0 0 -13791810 true "" "plot mean [riskPerceptionFromOthers] of orgs"
"pen-2" 1.0 0 -5298144 true "" "plot mean [riskPerceptionFromExp] of orgs"
...
480
EWorgs
count orgs with [extremeWeather?]
0
1
11
...
"" ""
PENS
"cumImpact" 1.0 0 -15040220 true "" "plot mean [sum ( impactExp)] of orgs"
"cumProb" 1.0 0 -8053223 true "" "plot mean [problem] of orgs"
...
430
unhappy
count orgs with [satisfied? =  false]
0
1
11
